...
# 11주차
* [계원 산업체 RT 참가업체 명단](/etc/2025/11/09/rt.html){:target="_blank"}
* 포트폴리오 콘텐츠 맵(Content Map)

## 포트폴리오 콘텐츠 맵 (Portfolio Content Map)

| 구분 | 섹션명 | 주요 콘텐츠 | 데이터 활용 요소 | 목적 / 설명 |
|------|--------|--------------|------------------|--------------|
| 1 | **Introduction** | 자기소개, 포트폴리오 개요, 제작 목적 | 인적 데이터 요약 (경력, 분야, 기술 스택 등) | 포트폴리오의 방향성과 핵심 역량 개요 제시 |
| 2 | **Career & Skill Overview** | 기술 스택 정리, 역량 그래프, 성장 타임라인 | 프로젝트별 기술 사용 빈도 데이터 | 기술적 강점과 성장 흐름을 시각화 |
| 3 | **Project Catalogue** | 주요 프로젝트 3~5개 | 프로젝트 메타데이터 (기간, 팀 규모, 성과 수치 등) | 핵심 프로젝트를 데이터 기반으로 정리 |
| 4 | **Process & Workflow** | 작업 절차, 협업 방식, 도구 사용 프로세스 | 업무 로그, 협업 툴 데이터 | 실제 작업 흐름과 효율성 분석 |
| 5 | **Result & Impact** | 프로젝트 성과, 지표 중심 결과 | 사용자 수, 성능 개선률, 리뷰 데이터 | 데이터로 검증된 성과를 시각화 |
| 6 | **Reflection & Growth** | 문제 해결 사례, 개선 과정, 배운 점 | 비교 데이터 (Before vs After) | 경험에서 도출한 학습 및 발전 내용 정리 |
| 7 | **Appendix / Reference** | 관련 자료, 링크, 코드 저장소 | 외부 레퍼런스 데이터 | 참고 자료와 추가 증빙 제공 |

## 역량 그래프(skill chart)
* [skill chart 역량 그래프](/etc/2025/11/09/skill_chart.html){:target="_blank"}
...
# 11주차 과제
* 포트폴리오 콘텐츠 맵 작성
  * 예시를 확인하고 자신에 맞는 내용 적용, 필요없는 내용 삭제, 예시에 없는 내용중 필요하다고 생각되는 내용 추가작성
  * 이는 포트폴리오 사이트의 목차(순서)가 됩니다.
* 역량 그래프 완성
  * 역량 그래프 내용을 참고하여 막연한 수치가 아닌 명확한 근거를 바탕으로 한 수치 작성
  * 내용을 작성하고 시각적으로 어떻게 표현할지도 같이 고민 해보세요.

---

# 12주차 - 메인 페이지 및 자기소개(Introduction) 완성
* 2025-11-17
  
## 주요 활동
  * 11주차 '콘텐츠 맵'을 바탕으로 메인(랜딩) 페이지 UI 구현.
  * 'Introduction' 섹션 내용 작성 및 디자인 (자기소개, 포부, 핵심 역량 요약).
  * 11주차 '역량 그래프(Skill Chart)'를 보완.
  
## 과제
  * 매주 동일하게 포트폴리오 링크 제출, 중간과정 스크린샷 5장 필수
  * 메인 페이지와 자기소개 섹션을 구현
  * *기획 의도를 담은 키 비주얼과 시각화된 역량 그래프를 포함*
...
## 주요 활동
  * 'Project Catalogue'에 따라 가장 자신 있는 핵심 프로젝트 2~3개의 상세 페이지 제작.
  * 각 프로젝트 페이지에 구체적인 내용 채우기 (프로젝트 개요, 기간, 역할, 사용 기술, 결과물 스크린샷/영상, 배운 점).
  * GitHub 리포지토리 링크 또는 라이브 데모 링크 추가.
  * 프론트엔드 취업, 웹 퍼블리싱 취업의 경우 '소스코드 리뷰'추가.
  
## 과제
  * 매주 동일하게 포트폴리오 링크 제출, 중간과정 스크린샷 5장 필수
  * 최소 2개 이상의 프로젝트 상세 페이지 포함
  * *프로젝트 개요, 적용 기술, 본인의 역할, 결과물(이미지/영상)을 상세히 기록*

---

# 14주차 - 서브 콘텐츠 제작 및 전체 검수
* 2025-12-01
  
## 주요 활동
  * '콘텐츠 맵'의 나머지 섹션(e.g., Career, Reflection, Contact) 페이지 제작 및 내용 작성.
  * 전체 페이지 간의 네비게이션(이동)이 부드럽게 이어지는지 확인.
  * *1차 검수 진행 (오탈자, 깨진 링크, 이미지 로딩 등).*
  
## 과제
  * 매주 동일하게 포트폴리오 링크 제출, 중간과정 스크린샷 5장 필수
  * 모든 목차가 구현되고 내용이 채워진 포트폴리오 1차 완성
  * 최소 모든 프로젝트 상세 페이지 포함
  * *다른 디바이스에서의 테스트 및 오탈자, 깨진 링크 등 자체 검수*
...
## 주요 활동
* 14주차까지의 작업을 바탕으로 최종 수정 및 보완.
* 기말 평가 기준에 맞춰 최종 포트폴리오 온라인 링크 제출.
* 기말평가를 위한 발표 등은 없습니다. 제출한 포트폴리오를 통하여 기말평가(상대평가) 진행합니다.
  
## 기말평가
* 완성된 포트폴리오 링크 제출,
* 기말평가 제출용 PDF파일 (웹사이트를 '인쇄'에서 PDF로 저장)
* 이력서, 자기소개서를 포트폴리오 사이트에 링크 또는 서브페이지로 수록
* 플랫폼 특성상, 희망직군 특성상 구현 불가능/미구현 한 부분은 카페 게시글로 작성 (예 : notion에서 폰트크기를 능동적으로 바꿀수 없었습니다. 영상관련 직무 포트폴리오로 코드리뷰가 필요하지 않았습니다.)

## 평가내용

| 구분 | 평가 항목 | 세부 기준 | 배점 예시 (5점 만점) |
|:----:|:---|:---|:--------------------:|
| 1 | 포트폴리오의 완성도 | 디자인, 레이아웃, 사용자 경험(UX) 등 전체적인 완성도 | 0~5 |
| 2 | 프로젝트 상세설명 | 프로젝트의 목표, 과정, 결과, 코드리뷰 등 내용의 구체성 | 0~5 |
| 3 | 내용의 정확성 | 링크 오류, 오탈자 등 콘텐츠의 신뢰도 | 0~5 |
| 4 | 콘텐츠의 양과 질 | 수록된 프로젝트의 수와 각 프로젝트의 깊이 | 0~5 |
| 5 | 직무 연관성 | 희망 직군과 포트폴리오 콘텐츠의 논리적 연결성 | 0~5 |
...
## 사후진단평가 
* <사후진단평가> 참여 방법
  * ▶ 1학년 수강생의 경우 학생성공지원시스템(excellent.kaywon.ac.kr) 로그인 → 교과목 진단평가 [평가하기] 클릭 후 진행
  * ▶ 2학년 수강생의 경우 기존 교육과정관리시스템(ncs.kaywon.ac.kr)로그인 → 교과목 진단평가 [평가하기] 클릭 후 진행

## 제출 파일 확인
* <div style="color:red">링크 다시한번 확인하기</div> - 링크로 접속이 불가능할시 미 제출에 가까운 패널티를 받게 됩니다.
* <div style="color:red">이럭서 PDF, 자기소개서 PDF파일 추가 첨부</div>

---

## 장래, 당부하고 싶은말
* 지속적으로 공부하기, 특히 AI관련 공부를 꼭 해보시길 바랍니다.
* [AI Developer 인공지능과 개발자의 미래](/etc/2025/12/07/ai_developer.html){:target="_blank"}

### 미래를 걱정하지마세요.
* 이미 증기기관으로 인한 1차 산업혁명과 대량의 실업, 러다이트 운동들이 일어났습니다.
* 이를 비유하여 4차 산업혁명과 대량의 실업을 예견하거나 불안감을 주는 정보들도 많이 있습니다.
* 1차 산업혁명은 자본가(공장 주인, 땅주인)들 만이 생산성을 폭발적으로 높일 수 있었습니다.
* 4차 산업혁명은 1차 만큼 폐쇄적이지 않습니다. ***핵심 기술에 접근할 수 있는 채널이 과거보다 훨씬 많습니다.***
  
### 공부를 지속하세요.
* 오늘날 누구나 온라인을 통해 최신 기술을 학습할 수 있는 기회가 열려 있습니다.
* 공부하고자 하면 공장을 직접 소유하지 못하더라도 ***AI라는 거대한 자동화 공장을 누구나 쓸 수 있습니다.***

### 쉽지는 않을 것입니다.
* '신입을 뽑지 않는다.','경력직도 위험하다' 같은 말에 현혹되지 마시고, AI 활용법을 공부하고, AI를 사용하여 능력을 확장하세요.
* 신입이 경력직보다 더 높은 퍼포먼스를 낼 수 있는 시대이기도 합니다. 공부를 통하여 좋은 위치를 차지하기를 바랍니다.

### AI 시대 이전 부터 반복되는 문제
* ‘이 직업이 유망할까요? 제가 취업할 수 있을까요?’라는 질문에 대한 답은 언제나 같습니다. 준비되어 있다면 취업은 가능합니다.
* 기계가 물건을 찍어내는 시대에도 수공예 장인은 여전히 필요합니다. 중요한 것은 ***분야에 대한 불안보다, 살아남을 수 있는 기술***을 갖추는 것입니다.
* 항상 공부하고 앞서나가는 사람이 되었으면 좋겠습니다.
...
# 12주차 ~ 15주차 목차 제안 (제작 기능 위주)

## 12주차 - 고급 데이터 연동 및 관리
* **주요 활동:**
    * 외부 API(GitHub 등)를 활용한 프로젝트 데이터 자동화
    * Headless CMS(Contentful 등)를 연동하여 포트폴리오 콘텐츠 관리
    * 전역 상태 관리 라이브러리(Zustand, Recoil 등)를 이용한 복잡한 상태 관리
* **과제:**
    * 1개 이상의 프로젝트를 API 또는 CMS와 연동하여 동적으로 표시되도록 구현

---

## 13주차 - 사용자 경험(UX) 및 인터랙션 심화
* **주요 활동:**
    * CSS 고급 기법(애니메이션, 트랜지션)을 활용한 동적인 UI 구현
    * React-Three-Fiber 등을 이용한 3D 모델 또는 인터랙티브 이펙트 추가
    * 포트폴리오 웹 접근성(Accessibility) 향상 (WAI-ARIA 표준 준수)
* **과제:**
    * 포트폴리오에 1개 이상의 고급 인터랙션 또는 3D 요소 추가
    * 웹 접근성 진단 및 개선 보고서 제출

---

## 14주차 - 빌드, 배포 최적화 및 최종 테스트
* **주요 활동:**
    * React 성능 최적화 (Code splitting, lazy loading, `React.memo` 등)
    * 검색엔진최적화(SEO) 적용 (react-helmet, sitemap 생성)
    * CI/CD (GitHub Actions 등)를 이용한 자동 배포 파이프라인 구축
* **과제:**
    * Lighthouse, PageSpeed Insights를 이용한 성능 측정 및 개선 결과 제출
    * 최종 빌드 및 배포 자동화 완료
...
* **주요 활동:**
    * 완성된 포트폴리오 최종 제출
    * 개발 과정 및 적용된 기술을 정리한 `README.md` 또는 기술 블로그 포스트 작성
* **평가 주안점:**
    * 기술적 완성도 및 적용된 기능의 깊이
    * 코드 퀄리티, 프로젝트 구조의 적절성
    * 최종 결과물의 완성도 및 독창성
