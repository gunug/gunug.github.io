...
{
    private string image_prompt = "A drawing of a computer writing a test";
    private OpenAIAPI api;
    ...
    public async void makeImage()
    {
        image_url = "";
        image = null;
        var result = await api.ImageGenerations.CreateImageAsync(new ImageGenerationRequest(image_prompt, OpenAI_API.Models.Model.DALLE3, ImageSize._1024, "hd")).ConfigureAwait(false);
    ...
    public void OnGUI()
    {
        image_prompt = GUI.TextArea(new Rect(Screen.width - 400, 0, 400, 40), image_prompt);
        if (GUI.Button(new Rect(Screen.width-400, 40, 400, 40), "Make Image"))
        {
            makeImage();
        }
        if(image != null){
